      ...
      >
        <div class="flex items-center gap-2">
          ...
          <div class="flex flex-col">
            ...
            <span class="cryptocurrency-item-rate">{{ `${item.rate} USD` }}</span>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <span class="cryptocurrency-item-amount">
...
export default defineComponent({
  ...
  computed: {
    ...
    cryptocurrencyOptions(): ICryptocurrencyOption[] {
      return this.invoiceAddresses
        .filter((item: ICryptocurrency, index: number, array: Array<ICryptocurrency>) => {
          return array.findIndex((elem: ICryptocurrency) => {
            return elem.currency.split('.')[0] === item.currency.split('.')[0];
          }) === index;
        })
        .map((item: ICryptocurrency) => {
          const symbol = item.currency.split('.')[0];
          return {
            currency: symbol === 'USDT'
              ? `Tether (${symbol})`
              : `${item.currency.split('.')[1]} (${symbol})`,
            value: item.currency,
            amount: item.amount,
            rate: item.rate,
            symbol,
  ...
  methods: {
    ...mapMutations('invoice', ['setSelectedCryptocurrency', 'setSelectedNetwork']),
    image,
