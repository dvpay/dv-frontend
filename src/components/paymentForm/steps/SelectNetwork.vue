...
export default defineComponent({
  ...
  computed: {
    ...
    networkOptions(): INetworkOption[] {
      return this.invoiceAddresses
        .filter((item: ICryptocurrencyOption) => {
          return item.currency.split('.')[0] === 'USDT';
        })
        .map((item: ICryptocurrencyOption) => ({
          title: item.blockchain === 'tron'
            ? `TRC20 (${item.currency.split('.')[0]})`
            : item.blockchain === 'eth'
              ? `ERC20 (${item.currency.split('.')[0]})`
              : '?',
          value: item.currency,
          blockchain: item.currency.split('.')[1],
        }));
    },
  ...
  methods: {
    ...mapMutations('invoice', ['setSelectedNetwork']),
    image,
