<template>
  <div>
    ...
    <div class="card" v-if="!exchangeConnect && !connectNewExchange">
      ...
      <button class="button button-primary px-5 mt-8 py-6px"
              @click="connectNewExchange = !connectNewExchange">
        {{ $t('Connect') }}
      </button>
    </div>
    <div class="card" v-if="exchangeConnect || connectNewExchange">
      ...
      <ui-select class="w-400px h-38px mt-2" id="walletType" v-model="selectExchange"
                 :options="exchangeList"
                 @change="syncDataFromVuex"/>
      <form v-if="selectExchange"
            @submit.prevent="connectHandler(exchangeConnect ? 'loadAddress' : 'testConnect')"
            class="mt-8">
...
export default defineComponent({
  ...
  computed: {
    ...
    exchangeList(): any {
      return this.exchanges.map((item: { name: any; slug: any; }) => ({
        title: item.name,
        value: item.slug,
      }));
    },
  ...
  async created() {
    ...
    if (this.exchangeConnect) {
      await this.connectHandler('loadAddress');
    }
  },
});
</script>
